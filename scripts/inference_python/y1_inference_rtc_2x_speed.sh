...
# Keep per-action interpolation duration near 1/60 s.
INTERP_STEPS="5"
INTERP_FREQUENCY="300"

# With ~250 ms RTT at 60 Hz, delay is about 15 steps, so horizon/latency_k
# need to be larger than the 30 Hz profile.
RTC_EXECUTE_HORIZON="16"
...
echo "[INFO] Starting Y1 RTC inference (60 Hz test profile)"
echo "[INFO] HOST=${HOST}:${PORT}"
echo "[INFO] PROMPT=${PROMPT}"
echo "[INFO] CAMERA_TYPE=${CAMERA_TYPE}"
echo "[INFO] CAMERA_NAMES=${CAMERA_NAMES[*]}"
echo "[INFO] CONTROL_FREQUENCY=${CONTROL_FREQUENCY}"
echo "[INFO] INFERENCE_RATE=${INFERENCE_RATE}"
echo "[INFO] CHUNK_SIZE=${CHUNK_SIZE}"
echo "[INFO] INTERP_STEPS=${INTERP_STEPS}"
echo "[INFO] INTERP_FREQUENCY=${INTERP_FREQUENCY}"
echo "[INFO] RTC_MODEL_CHUNK_SIZE=${RTC_MODEL_CHUNK_SIZE}"
echo "[INFO] RTC_EXECUTE_HORIZON=${RTC_EXECUTE_HORIZON}"
echo "[INFO] LATENCY_K=${LATENCY_K}"
echo "[INFO] MIN_SMOOTH_STEPS=${MIN_SMOOTH_STEPS}"
