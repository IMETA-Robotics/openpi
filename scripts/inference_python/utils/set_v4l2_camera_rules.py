    ...
    try:
        ...
        for line in output.splitlines():
            if line.startswith("P: "):
                ...
                current_dev = {'dev_node': None, 'vendor': None, 'serial': '', 'name': '', 'subsystem': '', 'interface_num': '', 'id_path': '', 'v4l_caps': ''}
...
def capture_camera_images(cameras):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = Path.cwd() / "images" / timestamp
    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n正在抓拍相机图片，保存目录: {output_dir}")
    for index, cam in enumerate(cameras):
        camera = V4l2Camera(f"preview_{index}")
        try:
            camera.set_up(cam["dev"])
            image = None
            for _ in range(10):
            ...
            if image is None:
                print(f"[{index}] {cam['dev']} 抓拍失败: 未获取到图像")
                continue

            filename = (
                f"{index:02d}_"
                f"{sanitize_filename(Path(cam['dev']).name)}_"
                f"{sanitize_filename(cam['serial'])}_"
                f"{sanitize_filename(cam['name'])}.jpg"
            )
            save_path = output_dir / filename
            cv2.imwrite(str(save_path), cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
            print(f"[{index}] {cam['dev']} -> {save_path.name}")
        except Exception as e:
            print(f"[{index}] {cam['dev']} 抓拍失败: {e}")
        finally:
            camera.stop()
...
def main():
    ...
    image_dir = capture_camera_images(cameras)
    print(f"\n请先查看抓拍图片目录: {image_dir}")
