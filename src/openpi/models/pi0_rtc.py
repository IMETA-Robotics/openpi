...
class Pi0RTC(_model.BaseModel):
    ...
    def sample_actions(
        ...
        *,
        ...
        def rtc_step(carry):
            ...
            if provided_dim > 0:
                def _apply_mask(x_local):
                    ...
                    overwrite = jnp.where(mask_time, prev_chunk[..., :provided_dim], x_local[..., :provided_dim])
                    return x_local.at[..., :provided_dim].set(overwrite)

                x_t_for_denoise = jax.lax.cond(
                    jnp.asarray(mask_prefix_delay, dtype=jnp.bool_),
                    _apply_mask,
                    lambda x_local: x_local,
                    x_t_for_denoise,
                )
        ...
        enable_rtc = jnp.asarray(enable_rtc, dtype=jnp.bool_)

        # Integrate for exactly num_steps steps from t=1.0 -> 0.0 to match training.
        if use_rtc:
            def scan_step(carry, _):
                next_carry = jax.lax.cond(enable_rtc, rtc_step, base_step, carry)
                return next_carry, None
        else:
            def scan_step(carry, _):
                return base_step(carry), None
